...
type EntryType = 'user' | 'group' | 'ou' | 'zone' | 'record' | 'extension' | 'trunk' | 'principal' | 'realm' | 'domain' | 'key' | 'alias' | 'profile' | 'nas' | 'attribute' | 'voicemail' | 'sipAccount' | 'iaxAccount' | 'template' | 'certificate' | 'icapService' | 'signingPolicy';
...
export default function NewEntryPage({
  ...
  const renderForm = () => {
    ...
    switch (entryType) {
      ...
      // PowerDNS
      case 'zone':
        return <ZoneForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'record':
        return <RecordForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;

      // FreeRADIUS (profiles are used as users, NAS for clients)
      case 'profile':
        return <ProfileForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'nas':
        return <NASForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'attribute':
        return <AttributeForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;

      // Asterisk
      case 'extension':
        return <DialplanForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'trunk':
        return <SipAccountForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'voicemail':
        return <VoicemailForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'sipAccount':
        return <SipAccountForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'iaxAccount':
        return <IaxAccountForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;

      // Kerberos
      case 'principal':
        return <PrincipalForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'realm':
        return <RealmForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'policy':
        return <PolicyForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;

      // Netcrave
      case 'template':
        return <TemplateForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'certificate':
        return <CertificateForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'icapService':
        return <IcapServiceForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;

      // OpenDKIM
      case 'domain':
        return <DomainForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'key':
        return <SelectorForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'signingPolicy':
        return <SigningPolicyForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;

      // Sendmail
      case 'alias':
        return <AliasForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'map':
        return <MapForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
      case 'class':
        return <ClassForm onSubmit={handleSubmit} onCancel={() => router.push(`/${resolvedParams.service}`)} />;
    }
  };
