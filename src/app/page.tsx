...
export default function Home() {
  const [servicesWithStats, setServicesWithStats] = useState<
    (typeof services)[0] & { stats: ServiceStats; connected: boolean }[]
  >([]);
  ...
  const getStatusBadge = (connected: boolean) => (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${connected ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
    >
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 rounded-lg bg-blue-50 border border-blue-100 p-6">
          ...
          <p className="text-sm text-blue-700">
            Connection:{" "}
            <span className="font-semibold text-green-700">Active</span> | Host:{" "}
            <span className="font-mono text-gray-600">
              ldap.example.com:389
            </span>{" "}
            | Base DN:{" "}
            <span className="font-mono text-gray-600">dc=example,dc=com</span>
          </p>
        </div>
        ...
        <section className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Link
              href="/openldap/new?entryType=user"
              className="flex items-center p-4 rounded-lg border border-gray-200 hover:bg-blue-50 transition-colors"
            >
              <svg
            ...
            <Link
              href="/openldap/new?entryType=group"
              className="flex items-center p-4 rounded-lg border border-gray-200 hover:bg-blue-50 transition-colors"
            >
              <svg
                className="h-6 w-6 text-blue-600 mr-3"
            ...
            <Link
              href="/openldap/new?entryType=ou"
              className="flex items-center p-4 rounded-lg border border-gray-200 hover:bg-blue-50 transition-colors"
            >
              <svg
                className="h-6 w-6 text-blue-600 mr-3"
